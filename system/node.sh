...
DAEMON_ARGS="/home/ec2-user/roi+/index.js" # path to your node.js server/app
# NB: don't use ~/ in path
 
DESC="node.js http server" # whatever fancy description you like
 
NODEUSER="ec2-user:ec2-user" # USER who OWNS the daemon process (no matter whoever runs the init script)
# user:group (if no group is specified, the primary GID for that user is used)
 
LOCAL_VAR_RUN=/home/ec2-user/roi+/system # in case the init script is run by non-root user, you need to
